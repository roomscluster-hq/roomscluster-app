...
export const foldersApi = {
    ...
    getBreadcrumbs: async (folderId: string) => {
        const res = await client.get<{ data: Breadcrumb[] }>(`/folders/${folderId}/breadcrumbs`);
        return unwrap(res);
    },

    rename: async (id: string, name: string) => {
        const res = await client.patch<{ data: Folder }>(`/folders/${id}`, { name });
        return unwrap(res);
    },
    ...
    delete: async (id: string) => {
        const res = await client.delete<{ data: { message: string } }>(`/folders/${id}`);
        return unwrap(res);
    },
};