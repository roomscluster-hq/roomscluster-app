...
import { PortalSessionCard } from "@/components/portal/PortalSessionCard";
      ...
      <main className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-ink-900 font-medium">No sessions yet</p>
            <p className="text-sm text-ink-700/50 mt-1">
              Sessions from groups you&apos;re enrolled in will show up here.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {sessions.map((session) => (
              <PortalSessionCard key={session.id} session={session} />
            ))}
          </div>
        )}
      </main>
